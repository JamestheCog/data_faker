...
contain the source code behind the `send.exe` executable in the assessment's files - in case the assessment ever changes in the future and / or the code needs to
...
`send.exe` was built and / or want to verify that it's not malware, feel free to browse through the executable's code (which is all written in Golang - the language
...
Building your own `send.exe` is easy no thanks to Go's capabilities:
...
Use `send.exe` like you would with any other CLI application.
